...
class _BrnScrollAnchorTabWidgetState extends State<BrnAnchorTab> with SingleTickerProviderStateMixin {
  //用于控制 滑动
  ...
  List<Widget> _fillList() {
    ...
    if (widget.widgetIndexedBuilder != null) {
      for (int i = 0, n = widget.itemCount; i < n; i++) {
        Widget itemWidget = Container(key: _bodyKeyList[i], child: widget.widgetIndexedBuilder!(context, i));
  ...
  void fillOffset() {
    Offset globalToLocal = (_key.currentContext!.findRenderObject() as RenderBox).localToGlobal(Offset.zero);
...
class TabShowBar extends StatefulWidget {
  ...
  @override
  State<TabShowBar> createState() => _TabShowBarState();
}

class _TabShowBarState extends State<TabShowBar> {
  double offset = 0;
